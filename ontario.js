...
async function index() {
    ...
    try {
        ...
        fs.writeFileSync('ontario.json', jsonContent, 'utf-8');
    } catch (error) {
        console.log(error);
    }
}
...
async function getNewHomes() {
    let content = fs.readFileSync('./ontario.json');
    ...
    for (let i = 0; i < objContent.length; i++) {   // state
        ...
        for (let j = 0; j < cities.length; j++) {   //city
            ...
            for (let k = 0; k < htmlNewHomes.length; k++) {     //new construction
                ...
                let developerLink = $newHomes('.developer-name', '.development-misc-info', resUnits.data).attr('href')
                ...
                if (developerLink) {
                    ...
                    developerAddress = $developer(htmlDeveloperInfos[0]).text().trim().replace('Address: ', '');
                    developerPhone = $developer(htmlDeveloperInfos[1]).text().trim().replace('Telephone: ', '');
                }
                ...
                io.on('connection', function (socket) {
                    io.emit('scrap status', `stateName (${i + 1}/${objContent.length}) - cityName (${j + 1}/${cities.length}) - newHomeName (${k + 1}/${htmlNewHomes.length})(${objData.length})`)
                })
            ...
            converter.json2csv(newStates, (err, csv) => {
                ...
                if (htmlNewHomes.length > 0) {
                    fs.writeFileSync('./ontario.csv', csv)
    ...
    console.log('scraping finished. please check *** ontario.csv *** file');
}
